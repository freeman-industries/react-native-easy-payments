...
RCT_EXPORT_METHOD(confirmPayment: (NSDictionary *)methodData
...
{    
    ...
    if (self.hasGatewayParameters) {
        [self.gatewayManager confirmPayment:methodData[@"clientSecret"] paymentParams:paymentParams completion:^(NSString * _Nullable token, NSError * _Nullable error) {
                if (error) {
                    NSLog(@"Failed");
                    NSLog(@"%@", error);
